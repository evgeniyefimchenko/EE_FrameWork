...
trait properties_trait {
    ...
    public function edit_property_set($params = []) {
        ...
        if (in_array('id', $params)) {
            ...
            $property_set_data = (int) $id ? $this->models['m_properties']->get_property_set_data($id) : [];
        } else { // Не передан ключевой параметр id
            SysClass::return_to_main(200, ENV_URL_SITE . '/admin/user_edit/id/' . $this->logged_in);
        }
        /* view */
    ...
    public function property_set_delete($params = []) {
        ...
        if (in_array('id', $params)) {
            ...
            if (count($res)) {
                Class_notifications::add_notification_user($this->logged_in, ['text' => 'Ошибка удаления свойства id=' . $set_id . '<br/>' . $res['error'], 'status' => 'danger']);
            } else {
                Class_notifications::add_notification_user($this->logged_in, ['text' => 'Удалено!', 'status' => 'info']);
            }
        } else {
