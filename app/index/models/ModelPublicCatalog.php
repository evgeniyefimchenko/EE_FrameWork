...
class ModelPublicCatalog {
    ...
    private array $previewCategoryRowCache = [];
    private array $previewPageRowCache = [];
    ...
    public function getObjectSearchPayload(string $query, string $languageCode = ENV_DEF_LANG, int $limit = 80): array {
        $languageCode = trim($languageCode) !== '' ? trim($languageCode) : ENV_DEF_LANG;
        $query = $this->normalizePublicSearchQuery($query);
        ...
        $rows = SafeMySQL::gi()->getAll(
            ...
            $languageCode,
            $languageCode,
            ['Resorts', 'Курорты', 'Тип записи: Objects', 'Objects', 'Объекты'],
            'Запись #%',
            5000
        ) ?: [];
        ...
        foreach ($rows as $row) {
            ...
            $categoryTitle = $this->decodeDisplayText((string) ($row['category_title'] ?? ''));
            if (preg_match('/^Без категории/i', $categoryTitle)) {
                $categoryTitle = '';
            }
            $cards[] = [
                'page_id' => $pageId,
                'title' => $this->decodeDisplayText((string) ($row['title'] ?? '')),
                ...
                'services' => array_values(array_filter((array) ($preview['services'] ?? []))),
                'food' => array_values(array_filter((array) ($preview['food'] ?? []))),
                'transfer' => array_values(array_filter((array) ($preview['transfer'] ?? []))),
                'category_title' => $categoryTitle,
            ];
        }

        $filteredCards = $query !== ''
            ? array_values(array_filter($cards, fn(array $card): bool => $this->publicObjectMatchesSearch($card, $query)))
            : [];

        return [
            'view_type' => 'search',
            'view_template' => 'v_public_search',
            'title' => 'Поиск объектов размещения',
            'meta_description' => 'Поиск гостиниц, отелей, гостевых домов, квартир и частного сектора.',
            'query' => $query,
            'pages' => array_slice($filteredCards, 0, $limit),
            'pages_total' => count($cards),
            'filtered_pages_total' => count($filteredCards),
            'canonical_url' => ENV_URL_SITE . '/search' . ($query !== '' ? ('?q=' . rawurlencode($query)) : ''),
            'alternate_links' => [],
    ...
    public function getPagePreviewPayload(int $pageId, string $languageCode = ENV_DEF_LANG): ?array {
        return $this->buildPagePayload($pageId, $languageCode, true);
    }

    public function getPagePayload(int $pageId, string $languageCode = ENV_DEF_LANG): ?array {
        return $this->buildPagePayload($pageId, $languageCode, false);
    }

    public function getCategoryPreviewPayload(int $categoryId, string $languageCode = ENV_DEF_LANG): ?array {
        return $this->buildCategoryPayload($categoryId, $languageCode, true);
    }

    public function getCategoryPayload(int $categoryId, string $languageCode = ENV_DEF_LANG): ?array {
        return $this->buildCategoryPayload($categoryId, $languageCode, false);
    }

    private function buildPagePayload(int $pageId, string $languageCode, bool $allowInactive): ?array {
        $basePayload = $this->getEntityBasePayload('page', $pageId, $languageCode, $allowInactive);
        if (!$basePayload) {
            return null;
        }
        ...
        $contactsPublicAllowed = !empty($owner['personal_data_distribution_consent_accepted'])
            && trim((string) ($pageRow['status'] ?? '')) === 'active';
        ...
        $map = $this->extractMapData($propertyMap['Карта'] ?? null, $this->decodeDisplayText((string) ($pageRow['title'] ?? '')));
        ...
        return array_merge($basePayload, [
            ...
            'resort' => [
                'title' => $this->decodeDisplayText((string) ($categoryRow['title'] ?? '')),
            ...
            'contacts' => [
                ...
                'website' => $website,
                'address' => $address,
                'public_allowed' => $contactsPublicAllowed,
            ],
            'location' => [
    ...
    private function buildCategoryPayload(int $categoryId, string $languageCode, bool $allowInactive): ?array {
        $basePayload = $this->getEntityBasePayload('category', $categoryId, $languageCode, $allowInactive);
        if (!$basePayload) {
            return null;
        }
        ...
        $map = $this->extractMapData($propertyMap['Карта курорта'] ?? null, $this->decodeDisplayText((string) ($categoryRow['title'] ?? '')));
        $resortBlocks = [];
        foreach ([
            'Блок текста: Жилье',
        ...
        $pageCards = $this->getPageCardsByCategory($categoryId, $languageCode, 0, 0);
        $filterState = $this->getPublicObjectFilterState();
        $filteredPageCards = $this->applyPublicObjectFilters($pageCards, $filterState);
        ...
        return array_merge($basePayload, [
            ...
            'summary' => $summary,
            'meta_description' => $metaDescription !== '' ? $metaDescription : (string) ($basePayload['meta_description'] ?? ''),
            'locative_title' => $locativeTitle !== '' ? $this->decodeDisplayText($locativeTitle) : $this->decodeDisplayText((string) ($categoryRow['title'] ?? '')),
            ...
            'pages' => $filteredPageCards,
            'pages_total' => count($pageCards),
            'filtered_pages_total' => count($filteredPageCards),
            'object_filters' => $this->buildPublicObjectFilters($pageCards, $filterState),
            'object_search' => [
                'query' => (string) ($filterState['q'] ?? ''),
            ],
            'object_type_groups' => $this->getCategoryObjectTypeGroups($categoryId, $languageCode),
        ]);
    }

    private function getEntityBasePayload(string $entityType, int $entityId, string $languageCode, bool $allowInactive): ?array {
        if (!$allowInactive) {
            $basePayload = EntityPublicUrlService::getEntityViewPayload($entityType, $entityId, $languageCode);
            return is_array($basePayload) ? $this->normalizeEntityDisplayPayload($basePayload) : null;
        }

        $basePayload = EntityPublicUrlService::getEntityViewPayload($entityType, $entityId, $languageCode);
        if (is_array($basePayload)) {
            return $this->normalizeEntityDisplayPayload($basePayload);
        }

        $previewPayload = $this->buildPreviewEntityBasePayload($entityType, $entityId, $languageCode);
        return is_array($previewPayload) ? $this->normalizeEntityDisplayPayload($previewPayload) : null;
    }

    private function buildPreviewEntityBasePayload(string $entityType, int $entityId, string $languageCode): ?array {
        $row = $entityType === 'category'
            ? $this->getPreviewCategoryRow($entityId)
            : $this->getPreviewPageRow($entityId);
        if (!is_array($row)) {
            return null;
        }

        $effectiveLanguageCode = trim((string) ($row['language_code'] ?? $languageCode));
        ...
        $path = $this->buildPreviewEntityPath($entityType, $row, $effectiveLanguageCode);

        return [
            'entity_type' => $entityType,
            'entity_id' => $entityId,
            'language_code' => $effectiveLanguageCode !== '' ? $effectiveLanguageCode : $languageCode,
            'title' => $title,
            ...
            'breadcrumbs' => $entityType === 'category'
                ? $this->buildPreviewCategoryBreadcrumbs((int) ($row['category_id'] ?? 0))
                : $this->buildPreviewPageBreadcrumbs((int) ($row['page_id'] ?? 0)),
            'meta_title' => $title,
            'meta_description' => $shortDescription !== '' ? $shortDescription : SysClass::truncateString($plainText, 180),
        ];
    }

    private function normalizeEntityDisplayPayload(array $payload): array {
        foreach (['title', 'meta_title', 'short_description', 'meta_description', 'plain_text'] as $fieldName) {
            if (array_key_exists($fieldName, $payload)) {
                $payload[$fieldName] = $this->decodeDisplayText((string) $payload[$fieldName]);
            }
        }

        if (!empty($payload['breadcrumbs']) && is_array($payload['breadcrumbs'])) {
            foreach ($payload['breadcrumbs'] as &$breadcrumb) {
                if (is_array($breadcrumb) && array_key_exists('title', $breadcrumb)) {
                    $breadcrumb['title'] = $this->decodeDisplayText((string) $breadcrumb['title']);
                }
            }
            unset($breadcrumb);
        }
    ...
    private function getPreviewPageRow(int $pageId): ?array {
        if ($pageId <= 0) {
            return null;
        }
        if (array_key_exists($pageId, $this->previewPageRowCache)) {
            return $this->previewPageRowCache[$pageId];
        }

        $row = SafeMySQL::gi()->getRow(
            'SELECT page_id, category_id, title, language_code, short_description, description, status, slug, route_path
             FROM ?n
             WHERE page_id = ?i
             LIMIT 1',
            Constants::PAGES_TABLE,
        ...
        $this->previewPageRowCache[$pageId] = is_array($row) ? $row : null;
        return $this->previewPageRowCache[$pageId];
    }

    private function getPreviewCategoryRow(int $categoryId): ?array {
        if ($categoryId <= 0) {
            return null;
        }
        if (array_key_exists($categoryId, $this->previewCategoryRowCache)) {
            return $this->previewCategoryRowCache[$categoryId];
        }

        $row = SafeMySQL::gi()->getRow(
            'SELECT category_id, parent_id, title, language_code, short_description, description, status, slug, route_path
             FROM ?n
             WHERE category_id = ?i
             LIMIT 1',
            Constants::CATEGORIES_TABLE,
        ...
        $this->previewCategoryRowCache[$categoryId] = is_array($row) ? $row : null;
        return $this->previewCategoryRowCache[$categoryId];
    }

    private function buildPreviewEntityPath(string $entityType, array $row, string $languageCode): string {
        $routePath = EntitySlugService::normalizeRoutePath((string) ($row['route_path'] ?? ''));
        if ($routePath !== '') {
            return $routePath;
        }

        if ($entityType === 'category') {
            return $this->buildPreviewCategoryPath((int) ($row['category_id'] ?? 0), $languageCode);
        }

        $categoryPath = $this->buildPreviewCategoryPath((int) ($row['category_id'] ?? 0), $languageCode);
        $slug = trim((string) ($row['slug'] ?? ''));
        if ($categoryPath === '' || $slug === '') {
            return '';
        }

        return rtrim($categoryPath, '/') . '/' . rawurlencode($slug);
    }

    private function buildPreviewCategoryPath(int $categoryId, string $languageCode): string {
        $visited = [];
        $segments = [];
        ...
        while ($currentId > 0 && !isset($visited[$currentId])) {
            ...
            $row = $this->getPreviewCategoryRow($currentId);
            if (!is_array($row)) {
                break;
            }
            ...
            if ($routePath !== '') {
                ...
                if ($prefix !== '') {
                    $segments = array_merge(array_values(array_filter(explode('/', $prefix))), array_reverse($segments));
                }
                break;
            }
    ...
    private function buildPreviewCategoryBreadcrumbs(int $categoryId): array {
        $breadcrumbs = [];
        $visited = [];
        $currentId = $categoryId;
    ...
    private function buildPreviewPageBreadcrumbs(int $pageId): array {
        $pageRow = $this->getPreviewPageRow($pageId);
        if (!is_array($pageRow)) {
            return [];
        }

        $breadcrumbs = $this->buildPreviewCategoryBreadcrumbs((int) ($pageRow['category_id'] ?? 0));
        $breadcrumbs[] = [
            'entity_type' => 'page',
            'entity_id' => (int) ($pageRow['page_id'] ?? 0),
            'title' => $this->decodeDisplayText((string) ($pageRow['title'] ?? '')),
            'url' => '',
        ];
    ...
    private function buildBlogCategoryPayload(array $basePayload, array $categoryRow, string $languageCode): array {
        ...
        $baseUrl = (string) ($basePayload['canonical_url'] ?? '');
        $articles = $this->getContentPageCardsByCategory((int) ($categoryRow['category_id'] ?? 0), $languageCode, 0, $perPage, true, $offset);
        $recentArticles = $this->getContentPageCardsByCategory((int) ($categoryRow['category_id'] ?? 0), $languageCode, 0, 5, true, 0);
        if (!empty($articles[0]['image_url'])) {
            $heroImage = (string) $articles[0]['image_url'];
        } elseif (!empty($recentArticles[0]['image_url'])) {
            $heroImage = (string) $recentArticles[0]['image_url'];
        }
        ...
        return array_merge($basePayload, [
            ...
            'summary' => $summary,
            'meta_description' => $metaDescription !== '' ? $metaDescription : (string) ($basePayload['meta_description'] ?? ''),
            'canonical_url' => $this->buildPagedPublicUrl($baseUrl, $currentPage),
            'overview_html' => $descriptionHtml,
            'hero_image' => $heroImage,
            'articles' => $articles,
            'recent_articles' => $recentArticles,
            'pagination' => $this->buildPublicPaginationPayload($baseUrl, $currentPage, $totalArticles, $perPage),
            'language_links' => $this->filterLanguageLinks((array) ($basePayload['alternate_links'] ?? [])),
        ]);
    }

    private function getContentPageCardsByCategory(int $categoryId, string $languageCode, int $excludePageId = 0, int $limit = 12, bool $includeDescendants = false, int $offset = 0): array {
        if ($categoryId <= 0) {
            return [];
        }
        ...
        $sql = 'SELECT p.page_id, p.title, p.slug, p.route_path, p.short_description, p.description, p.sort_order, p.created_at, p.updated_at, c.title AS category_title
                FROM ?n AS p
                JOIN ?n AS c ON c.category_id = p.category_id
        ...
        $sql .= ' ORDER BY p.sort_order ASC, p.created_at DESC, p.page_id DESC LIMIT ?i, ?i';
        $params[] = max(0, $offset);
        $params[] = max(1, $limit);
    ...
    private function countContentPagesByCategory(int $categoryId, string $languageCode, int $excludePageId = 0, bool $includeDescendants = false): int {
        if ($categoryId <= 0) {
            return 0;
        }
        ...
        $sql = 'SELECT COUNT(*)
                ...
                WHERE p.category_id IN (?a)
                  AND p.status = ?s
                  AND p.language_code = ?s';
        $params = [Constants::PAGES_TABLE, $categoryIds, 'active', $languageCode];
        if ($excludePageId > 0) {
            $sql .= ' AND p.page_id != ?i';
            $params[] = $excludePageId;
        }
    ...
    private function buildPublicPaginationPayload(string $baseUrl, int $currentPage, int $totalItems, int $perPage): array {
        $totalPages = max(1, (int) ceil(max(0, $totalItems) / max(1, $perPage)));
        ...
        foreach ($pageNumbers as $pageNumber) {
            ...
            $links[] = [
                ...
                'href' => $this->buildPagedPublicUrl($baseUrl, $pageNumber),
        ...
        return [
            ...
            'prev_href' => $currentPage > 1 ? $this->buildPagedPublicUrl($baseUrl, $currentPage - 1) : '',
            'next_href' => $currentPage < $totalPages ? $this->buildPagedPublicUrl($baseUrl, $currentPage + 1) : '',
            'links' => $links,
        ];
    }

    private function buildPagedPublicUrl(string $baseUrl, int $page): string {
        $baseUrl = trim($baseUrl);
        if ($baseUrl === '') {
            $baseUrl = (string) ENV_URL_SITE;
        }
    ...
    private function getRecentContentPageCardsByTypeNames(array $typeNames, string $languageCode, int $excludePageId = 0, int $limit = 5): array {
        ...
        $sql .= ' ORDER BY p.sort_order ASC, p.created_at DESC, p.page_id DESC LIMIT ?i';
        $params[] = max(1, $limit);
    ...
    private function buildContentPageCard(array $row, string $languageCode): array {
        ...
        return [
            ...
            'category_title' => $this->decodeDisplayText((string) ($row['category_title'] ?? '')),
        ];
    }
    ...
    private function getPageOwner(int $pageId): array {
        LegalConsentService::ensureDistributionInfrastructure();
        $row = SafeMySQL::gi()->getRow(
            'SELECT pu.user_id, u.name, u.email, u.phone, u.personal_data_distribution_consent_accepted
             FROM ?n AS pu
        ...
        return [
            ...
            'personal_data_distribution_consent_accepted' => !empty($row['personal_data_distribution_consent_accepted']) ? 1 : 0,
        ];
    }
    ...
    private function getPublicObjectFilterState(): array {
        return [
            'q' => $this->normalizePublicSearchQuery($_GET['q'] ?? ''),
            'object_type' => $this->normalizePublicFilterList($_GET['object_type'] ?? []),
            'services' => $this->normalizePublicFilterList($_GET['services'] ?? []),
            'food' => $this->normalizePublicFilterList($_GET['food'] ?? []),
            'transfer' => $this->normalizePublicFilterList($_GET['transfer'] ?? []),
            'price_min' => $this->normalizePublicFilterNumber($_GET['price_min'] ?? null),
            'price_max' => $this->normalizePublicFilterNumber($_GET['price_max'] ?? null),
            'sea_max' => $this->normalizePublicFilterNumber($_GET['sea_max'] ?? null),
        ];
    }

    private function normalizePublicSearchQuery(mixed $value): string {
        if (is_array($value)) {
            $value = reset($value);
        }

        $value = html_entity_decode(trim(strip_tags((string) $value)), ENT_QUOTES | ENT_HTML5, 'UTF-8');
        $value = preg_replace('/\s+/u', ' ', $value) ?? $value;
        return trim(mb_substr($value, 0, 120, 'UTF-8'));
    }

    private function normalizePublicFilterList(mixed $value): array {
        $items = is_array($value) ? $value : [$value];
        $result = [];
        $seen = [];
        foreach ($items as $item) {
            $item = html_entity_decode(trim(strip_tags((string) $item)), ENT_QUOTES | ENT_HTML5, 'UTF-8');
            $item = preg_replace('/\s+/u', ' ', $item) ?? $item;
            $item = trim(mb_substr($item, 0, 120, 'UTF-8'));
            if ($item === '') {
                continue;
            }
            $key = $this->normalizePublicFilterKey($item);
    ...
    private function normalizePublicFilterNumber(mixed $value): ?int {
        if (is_array($value)) {
            $value = reset($value);
        }
        ...
        $number = (int) floor((float) $matches[0]);
        return $number >= 0 ? $number : null;
    }

    private function normalizePublicFilterKey(string $value): string {
        $value = html_entity_decode(trim(strip_tags($value)), ENT_QUOTES | ENT_HTML5, 'UTF-8');
        $value = preg_replace('/\s+/u', ' ', $value) ?? $value;
        return mb_strtolower(trim($value), 'UTF-8');
    }

    private function applyPublicObjectFilters(array $pages, array $filterState): array {
        return array_values(array_filter($pages, function (array $page) use ($filterState): bool {
            $query = (string) ($filterState['q'] ?? '');
            if ($query !== '' && !$this->publicObjectMatchesSearch($page, $query)) {
                return false;
            }

            if (!$this->publicPageMatchesSelectedValues((array) ($filterState['object_type'] ?? []), [$page['object_type'] ?? ''])) {
                return false;
            }
            if (!$this->publicPageMatchesSelectedValues((array) ($filterState['services'] ?? []), $this->getPublicPageFilterValues($page, 'services'))) {
                return false;
            }
            if (!$this->publicPageMatchesSelectedValues((array) ($filterState['food'] ?? []), $this->getPublicPageFilterValues($page, 'food'))) {
                return false;
            }
            if (!$this->publicPageMatchesSelectedValues((array) ($filterState['transfer'] ?? []), $this->getPublicPageFilterValues($page, 'transfer'))) {
                return false;
            }

            $price = $this->extractPublicNumber((string) ($page['price_from'] ?? ''));
            $priceMin = $filterState['price_min'] ?? null;
            $priceMax = $filterState['price_max'] ?? null;
            if ($priceMin !== null && ($price === null || $price < (int) $priceMin)) {
                return false;
            }
            if ($priceMax !== null && ($price === null || $price > (int) $priceMax)) {
                return false;
            }

            $distanceToSea = $this->extractPublicNumber((string) ($page['distance_to_sea'] ?? ''));
            $seaMax = $filterState['sea_max'] ?? null;
            if ($seaMax !== null && ($distanceToSea === null || $distanceToSea > (int) $seaMax)) {
                return false;
            }
    ...
    private function publicObjectMatchesSearch(array $page, string $query): bool {
        $tokens = preg_split('/\s+/u', $this->normalizePublicFilterKey($query), -1, PREG_SPLIT_NO_EMPTY) ?: [];
        if ($tokens === []) {
            return true;
        }

        $haystack = $this->normalizePublicFilterKey(implode(' ', array_filter([
            (string) ($page['title'] ?? ''),
            (string) ($page['summary'] ?? ''),
            (string) ($page['category_title'] ?? ''),
            (string) ($page['object_type'] ?? ''),
            implode(' ', $this->getPublicPageFilterValues($page, 'services')),
            implode(' ', $this->getPublicPageFilterValues($page, 'food')),
            implode(' ', $this->getPublicPageFilterValues($page, 'transfer')),
        ])));

        foreach ($tokens as $token) {
            if ($token !== '' && mb_strpos($haystack, $token, 0, 'UTF-8') === false) {
                return false;
            }
        }
    ...
    private function publicPageMatchesSelectedValues(array $selectedValues, array $pageValues): bool {
        if ($selectedValues === []) {
            return true;
        }
        ...
        foreach ($pageValues as $pageValue) {
            $pageValue = trim((string) $pageValue);
            if ($pageValue !== '') {
                $pageKeys[$this->normalizePublicFilterKey($pageValue)] = true;
            }
        }
        if ($pageKeys === []) {
            return false;
        ...
        foreach ($selectedValues as $selectedValue) {
            $selectedKey = $this->normalizePublicFilterKey((string) $selectedValue);
            if ($selectedKey !== '' && isset($pageKeys[$selectedKey])) {
                return true;
            }
        }
    ...
    private function getPublicPageFilterValues(array $page, string $key): array {
        $value = $page[$key] ?? [];
        ...
        foreach ((array) $items as $item) {
            ...
            $filterKey = $this->normalizePublicFilterKey($item);
            if ($filterKey === '' || isset($seen[$filterKey])) {
                continue;
            }
            $seen[$filterKey] = true;
            $result[] = $item;
    ...
    private function extractPublicNumber(string $value): ?int {
    ...
    private function buildPublicObjectFilters(array $pages, array $filterState): array {
        $objectTypeCounts = [];
        $servicesCounts = [];
        $foodCounts = [];
        $transferCounts = [];
        ...
        foreach ($pages as $page) {
            ...
            $objectType = trim((string) ($page['object_type'] ?? ''));
            if ($objectType !== '') {
                $objectTypeCounts[$objectType] = (int) ($objectTypeCounts[$objectType] ?? 0) + 1;
            }

            foreach ($this->getPublicPageFilterValues($page, 'services') as $item) {
                $servicesCounts[$item] = (int) ($servicesCounts[$item] ?? 0) + 1;
            }
            foreach ($this->getPublicPageFilterValues($page, 'food') as $item) {
                $foodCounts[$item] = (int) ($foodCounts[$item] ?? 0) + 1;
            }
            foreach ($this->getPublicPageFilterValues($page, 'transfer') as $item) {
                $transferCounts[$item] = (int) ($transferCounts[$item] ?? 0) + 1;
            }
            ...
            $distance = $this->extractPublicNumber((string) ($page['distance_to_sea'] ?? ''));
            if ($distance !== null) {
                $distances[] = $distance;
            }
        }

        $activeCount = count((array) ($filterState['object_type'] ?? []))
            + count((array) ($filterState['services'] ?? []))
            + count((array) ($filterState['food'] ?? []))
            + count((array) ($filterState['transfer'] ?? []))
            + ((string) ($filterState['q'] ?? '') !== '' ? 1 : 0)
            + (($filterState['price_min'] ?? null) !== null ? 1 : 0)
            + (($filterState['price_max'] ?? null) !== null ? 1 : 0)
            + (($filterState['sea_max'] ?? null) !== null ? 1 : 0);

        return [
            'state' => $filterState,
            'active_count' => $activeCount,
            'object_type_options' => $this->buildPublicFilterOptions($objectTypeCounts, (array) ($filterState['object_type'] ?? [])),
            'services_options' => $this->buildPublicFilterOptions($servicesCounts, (array) ($filterState['services'] ?? [])),
            'food_options' => $this->buildPublicFilterOptions($foodCounts, (array) ($filterState['food'] ?? [])),
            'transfer_options' => $this->buildPublicFilterOptions($transferCounts, (array) ($filterState['transfer'] ?? [])),
            'price' => [
                'min_available' => $prices !== [] ? min($prices) : null,
                'max_available' => $prices !== [] ? max($prices) : null,
                'min' => $filterState['price_min'] ?? null,
                'max' => $filterState['price_max'] ?? null,
            ],
            'distance' => [
                'min_available' => $distances !== [] ? min($distances) : null,
                'max_available' => $distances !== [] ? max($distances) : null,
                'max' => $filterState['sea_max'] ?? null,
            ],
        ];
    }

    private function buildPublicFilterOptions(array $counts, array $selectedValues): array {
        if ($counts === []) {
            return [];
        }
        ...
        foreach ($counts as $label => $count) {
            ...
            $options[] = [
                ...
                'label' => $label,
                'count' => (int) $count,
                'checked' => isset($selectedKeys[$this->normalizePublicFilterKey($label)]),
            ];
        }
    ...
    private function getPublicCategoryRows(string $languageCode): array {
        ...
        foreach ($rows as &$row) {
            $row['title'] = $this->decodeDisplayText((string) ($row['title'] ?? ''));
        }
        unset($row);
    ...
    private function getPagePreviewPropertyMap(array $pageIds, string $languageCode): array {
        ...
        $servicesPropertyId = $this->getPropertyIdByName('Предоставляемые услуги');
        $foodPropertyId = $this->getPropertyIdByName('Питание');
        $transferPropertyId = $this->getPropertyIdByName('Трансфер');
        ...
        foreach ($rows as $row) {
            ...
            } elseif ($name === 'Предоставляемые услуги') {
                $map[$entityId]['services'] = $this->extractChoiceList($property);
            } elseif ($name === 'Питание') {
                $map[$entityId]['food'] = $this->extractChoiceList($property);
            } elseif ($name === 'Трансфер') {
                $map[$entityId]['transfer'] = $this->extractChoiceList($property);
            }
        }
