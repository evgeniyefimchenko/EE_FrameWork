...
ИИ-профили хранят секреты провайдеров в `ee_ai_profiles.encrypted_api_key`.

Правила:

- API-ключи не должны попадать в Git, документацию, issue tracker или публичные примеры;
- полный ключ не должен выводиться обратно в HTML после сохранения;
- UI показывает только `api_key_mask`;
- новое значение ключа шифруется через `AES-256-GCM` с `ENV_SECRET_KEY`;
- если `ENV_SECRET_KEY` не задан, сохранение нового ключа должно завершиться ошибкой;
- поле ключа в форме редактирования можно оставить пустым, чтобы сохранить уже записанный секрет.

Проверка подключения ИИ-профиля обращается к models endpoint провайдера и не должна отправлять пользовательские prompt-ы.
