...
## Мультиязычные страницы и категории

Для `pages` и `categories` мультиязычность строится не через одну запись с множеством полей, а через отдельные сущности на каждый язык:

- `ee_pages.language_code`
- `ee_categories.language_code`
- `ee_entity_translations` для связи языковых версий между собой

За orchestration этого слоя отвечает `EntityTranslationService`.

Практический workflow такой:

- модель сохраняет страницу или категорию в конкретной локали;
- после сохранения вызывается `EntityTranslationService::ensureEntity(...)`;
- при создании нового перевода используется `linkEntityToSource(...)`;
- property values копируются в новую языковую версию через `duplicatePropertyValuesFromSource(...)`.

Это позволяет вести три языка как три независимые записи, но при этом не терять связь между версиями в админке и сервисном коде.
