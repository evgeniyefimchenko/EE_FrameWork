...
Для публичного rich text это означает не просто “данные пришли из админки” и не просто импортную нормализацию. Такой HTML должен пройти allowlist sanitizer: опасные теги, event-атрибуты и URL-схемы вроде `javascript:`, `data:` и `vbscript:` не должны доходить до шаблона.
