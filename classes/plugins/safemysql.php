<? if (ENV_SITE !== 1) {header("HTTP/1.1 301 Moved Permanently");header("Location: http://". $_SERVER['HTTP_HOST']);exit();}?>
<?php
...
class SafeMySQL extends Singleton {
    ...
    /**
     * Функция проверки входного значения или массива значений на наличие в списке разрешенных.
     *
     * @param mixed $input Входное значение или массив для проверки.
     * @param array $allowed Список разрешенных значений.
     * @param mixed $default Значение по умолчанию, если входное значение не найдено в списке разрешенных.
     *
    ...
    private function escapeInt($value) {
        ...
        if (!is_numeric($value)) {
            $this->error("Целочисленный (?i) заполнитель ожидает числовое значение , " . gettype($value) . " было переданно");
            return FALSE;
        }
        if (is_float($value)) {
