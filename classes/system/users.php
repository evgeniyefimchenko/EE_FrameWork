...
class Users {
    ...
    private function getDefaultPropertyTypeSeeds(string $languageCode): array {
        if ($languageCode === 'EN') {
            return [
                ...
                ['name' => 'Date range', 'description' => 'Date interval field.', 'fields' => ['date-range']],
                ['name' => 'Repeatable group', 'description' => 'Nested repeatable group of fields.', 'fields' => ['repeatable-group']],
        ...
        return [
            ...
            ['name' => 'Интервал дат', 'description' => 'Поле интервала дат.', 'fields' => ['date-range']],
            ['name' => 'Повторяемая группа', 'description' => 'Вложенная повторяемая группа полей.', 'fields' => ['repeatable-group']],
    ...
    private function createUsersTable() {
        $sql = "CREATE TABLE IF NOT EXISTS ?n (
            ...
            personal_data_distribution_consent_accepted TINYINT(1) NOT NULL DEFAULT 0 COMMENT 'согласие на распространение ПДн',
            personal_data_distribution_consent_accepted_at DATETIME DEFAULT NULL,
            personal_data_distribution_consent_accept_ip VARCHAR(45) DEFAULT NULL,
            personal_data_distribution_consent_accept_user_agent VARCHAR(255) DEFAULT NULL,
            personal_data_distribution_consent_version VARCHAR(50) DEFAULT NULL,
    ...
    private function createUsersMessageTable() {
        $sql = "CREATE TABLE IF NOT EXISTS ?n (
           ...
           KEY idx_users_message_source (user_id, source_type, source_id),
           FOREIGN KEY (user_id) REFERENCES ?n(user_id) ON DELETE CASCADE,
